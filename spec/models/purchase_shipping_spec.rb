  ...
  before do
    ...
    context '商品購入ができない時' do
      ...
      it 'phoneが9桁以下では購入できない' do
        ...
        @purchase_shipping.valid?
        expect(@purchase_shipping.errors.full_messages).to include('Phone is too short (minimum is 10 characters)')
      end
      it 'phoneが12桁以上では購入できない' do
        @purchase_shipping.phone =  '123456789123'
        @purchase_shipping.valid?
        expect(@purchase_shipping.errors.full_messages).to include('Phone is too long (maximum is 11 characters)')
      end
